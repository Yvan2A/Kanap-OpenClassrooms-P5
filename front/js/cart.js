//Initialisation du local storage (Ajout de key et de values qui sont dans le localstorage)

let produitLocalStorage = JSON.parse(localStorage.getItem("produit")); /*On convertit les données au format JSON en objet JS et on utilise la méthode GET*/
console.table(produitLocalStorage);
const positionEmptyCart = document.querySelector("#cart__items");

    // ------------------- Création du DOM de la page panier -------------------------
...
// Si le panier contient un article // création du HTML dans le DOM
} else {
...
getCart();
    ...
    //-------------------- Calcul montant total + modification et suppression produit ------------------------------

// création de la fonction total
function getTotals(){
...
function deleteProduct() {
    ...
    for (let j = 0; j < btn_supprimer.length; j++){
        btn_supprimer[j].addEventListener("click" , (event) => {
            ...
            //Avec la méthode filter, je supprime le bouton sélectionné
...
function postForm(){
    ...
    btn_commander.addEventListener("click", ()=>{
        ...
        const options = {
            ...
            body: JSON.stringify(order), // j'indique que le body est l'objet (order) sous forme de string 
            headers: {
